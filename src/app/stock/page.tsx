...
import { Plus, Search, Edit3, Trash2, Package, Star } from 'lucide-react';
...
function StockContent() {
  ...
  const [catalogProducts, setCatalogProducts] = useState<Array<{ id: string; nameFr: string; nameWolof: string; category: string; unit: string; defaultPrice: number; sponsored?: boolean }>>([]);
  const [catalogSearch, setCatalogSearch] = useState('');
  const [editingProduct, setEditingProduct] = useState<VendorProduct | null>(null);
  const [editPrice, setEditPrice] = useState(0);
  const [editStock, setEditStock] = useState(0);
  const [customProduct, setCustomProduct] = useState({ name: '', nameWolof: '', unit: 'unité', price: 0, stock: 0 });
  ...
  useEffect(() => {
    if (showAddModal && addMode === 'catalog') {
      loadCatalog();
    }
  }, [showAddModal, addMode, catalogSearch]);
  ...
  const addCustomProduct = async () => {
    try {
      ...
      setShowAddModal(false);
      setCustomProduct({ name: '', nameWolof: '', unit: 'unité', price: 0, stock: 0 });
      loadProducts();
    } catch (err) {
  ...
  const removeProduct = async (id: string) => {
    if (!confirm('Retirer ce produit du rayon ?')) return;
    try {
      await apiFetch(`/api/vendor/products?id=${id}`, { method: 'DELETE' });
      loadProducts();
    } catch (err) {
          ...
          {filteredProducts.map((product) => (
            <div
              ...
              className={`bg-white rounded-xl shadow-sm p-4 ${
                product.stock <= product.lowStockAlert ? 'border-l-4 border-yellow-400' : ''
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-800">{product.productId.slice(0, 16)}...</p>
                    <p className="text-sm text-gray-500">Type: {product.productType}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold">{formatCurrency(product.price)}</p>
                  <p className={`text-sm ${product.stock <= product.lowStockAlert ? 'text-red-500 font-medium' : 'text-gray-500'}`}>
                    Stock: {product.stock}
                  </p>
                </div>
              </div>
              <div className="flex gap-2 mt-3 justify-end">
                <button
                  onClick={() => {
                    ...
                    setEditPrice(product.price / 100);
            ...
            <div className="bg-white rounded-2xl p-6 w-full max-w-md">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={updateProduct} className="flex-1 bg-[#10b981] text-white py-3 rounded-lg font-medium">
                    {t('common.save')}
                  </button>
                  <button onClick={() => setEditingProduct(null)} className="flex-1 bg-gray-100 py-3 rounded-lg">
                    {t('common.cancel')}
                  </button>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <button
                  ...
                  className={`py-2 rounded-lg text-sm font-medium ${
                    addMode === 'catalog' ? 'bg-[#1e3a8a] text-white' : 'bg-gray-100'
                  }`}
                >
                  {t('stock.fromCatalog')}
                </button>
                <button
                  ...
                  className={`py-2 rounded-lg text-sm font-medium ${
                    addMode === 'custom' ? 'bg-[#1e3a8a] text-white' : 'bg-gray-100'
                  }`}
                >
              ...
              {addMode === 'catalog' ? (
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {catalogProducts.map((p) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg border border-gray-100 text-left"
                      >
                        <div>
                          ...
                          <span className="text-xs text-gray-400">{p.category} | {p.unit}</span>
                        </div>
                ...
                <div className="space-y-3">
                  ...
                  <input
                    type="text"
                    placeholder={t('stock.productNameWolof')}
                    value={customProduct.nameWolof}
                    onChange={(e) => setCustomProduct({ ...customProduct, nameWolof: e.target.value })}
                    className="w-full px-4 py-3 border rounded-lg"
                  />
                  <select
                  ...
                  <input
                    type="number"
                    placeholder={`${t('stock.price')} (FCFA)`}
                  ...
                  <input
                    ...
                    onChange={(e) => setCustomProduct({ ...customProduct, stock: Number(e.target.value) })}
                    className="w-full px-4 py-3 border rounded-lg"
                  />
                  <button
                    onClick={addCustomProduct}
                    className="w-full bg-[#10b981] text-white py-3 rounded-lg font-medium"
                  >
                    {t('common.save')}
                  </button>
                </div>
